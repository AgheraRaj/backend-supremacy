    ...
    #commonFunction(inputObj, keys, isDecrypt) {
        for (let key in inputObj) {
            if (keys.includes(key) && typeof inputObj[key] === 'string') {
                ...
                if ( typeof inputObj[key] === 'object' || keys.includes(key)) {
                    if (isDecrypt) {
                        inputObj[key] = this.decryptObjectKeys(inputObj[key], keys)
                    } else {
                        inputObj[key] = this.encryptObjectKeys(inputObj[key], keys)
                    }
                }
            }
        }
